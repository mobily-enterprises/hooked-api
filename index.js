...
const ResourceProxyHandler = {
  get(target, prop) {
    ...
    if (resourceConfig && resourceConfig.implementers && resourceConfig.implementers.has(prop)) {
      ...
      // Resource-specific implementers are called directly with their own options building
      return async (context = {}) => {
        // Build options for resource-specific implementers
        const options = {
        ...
        // Check handler parameter count for compatibility
        if (resourceSpecificHandler.length <= 1) {
          // Legacy handler - inject properties
          context.resourceName = resourceName;
          context.apiInstance = apiInstance;
          const result = await resourceSpecificHandler(context);
          delete context.apiInstance;
          delete context.resourceName;
          return result;
        } else {
          // New style handler with clean separation
          return await resourceSpecificHandler(context, options);
        }
      };
    }
...
static _findApiInstanceByResourceName(resourceName, versionRange = 'latest') {
  ...
  async _executeWithResource(method, context, resourceName) {
    ...
    if (handler.length <= 1) {
      ...
      // New two-parameter handler - clean separation
